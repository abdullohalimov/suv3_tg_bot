...
def male_female_keyboard(lang):
    keyboard = InlineKeyboardBuilder()
    keyboard.add(InlineKeyboardButton(text=_("Эркак", locale=lang),
                 callback_data=Factories.MaleFemale(id='1').pack()))
    keyboard.add(InlineKeyboardButton(text=_("Аёл", locale=lang),
                 callback_data=Factories.MaleFemale(id='2').pack()))
    keyboard.add(InlineKeyboardButton(text=_("🔙 Орқага", locale=lang), callback_data=Factories.Back(id='back').pack()))
    keyboard.adjust(1)
    return keyboard.as_markup()
...
async def position_keyboard(lang):
    keyboard = InlineKeyboardBuilder()
    keyboard.add(InlineKeyboardButton(text=_("Хўжалик рахбари",
                 locale=lang), callback_data=Factories.Position(id=1).pack()))
    keyboard.add(InlineKeyboardButton(text=_("Бухгалтер", locale=lang),
                 callback_data=Factories.Position(id=2).pack()))
    keyboard.add(InlineKeyboardButton(text=_("Ишчи", locale=lang),
...
async def faoliyat_turi_keyboard(lang):
    keyboard = InlineKeyboardBuilder()
    keyboard.add(InlineKeyboardButton(text=_("Пахтачилик/ғаллачилик",
                 locale=lang), callback_data=Factories.FaoliyatTuri(id=1).pack()))
    keyboard.add(InlineKeyboardButton(text=_("Боғдорчилик/узумчилик",
                 locale=lang), callback_data=Factories.FaoliyatTuri(id=2).pack()))
    keyboard.add(InlineKeyboardButton(text=_("Сабзавот-полиз", locale=lang),
                 callback_data=Factories.FaoliyatTuri(id=3).pack()))
    keyboard.add(InlineKeyboardButton(text=_("Сабзавот-ғалла", locale=lang),
                 callback_data=Factories.FaoliyatTuri(id=1).pack()))
    keyboard.add(InlineKeyboardButton(text=_("Бошқа йўналиш", locale=lang),
                 callback_data=Factories.FaoliyatTuri(id=1).pack()))
    keyboard.add(InlineKeyboardButton(text=_("🔙 Орқага", locale=lang), callback_data=Factories.Back(id='back').pack()))
...
async def download_cert(lang):
    keyboard = InlineKeyboardBuilder()
    keyboard.add(InlineKeyboardButton(text=_("Сертификатни юклаб олиш",
                 locale=lang), callback_data=Factories.Certificate(id='download').pack()))
    keyboard.adjust(1)
...
async def region_inline_keyboard(lang):
    ...
    for i in regions_list['data']:
        ...
        keyb.add(InlineKeyboardButton(
            text=str(i['name']), callback_data=Factories.Region(id=str(i['id'])).pack()))
    keyb.adjust(2)
    keyb.add(InlineKeyboardButton(text=_("🔙 Орқага", locale=lang), callback_data=Factories.Back(id='back').pack()))
    ...
    for i in districts_list['data']:
        ...
        if str(i['id']) == str(region_id):
            for j in i['districts']:
                logging.info(j)
                keyb.add(InlineKeyboardButton(
                    text=str(j['name']), callback_data=Factories.District(id=str(j['id'])).pack()))
    keyb.adjust(2)
    keyb.add(InlineKeyboardButton(text=_("🔙 Орқага", locale=lang), callback_data=Factories.Back(id='back').pack()))
